...
const HoatDongComponent = () => {
    ...
    useEffect(() => {
        const fetchActivities = async () => {
            try {
                ...
                setSnackbarMessage("Không thể tải danh sách hoạt động. Vui lòng thử lại!");
    ...
    useEffect(() => {
        ...
        if (userCookie) {
            try {
                ...
                setLoading(false);
                setSnackbarMessage("Lỗi khi tải thông tin người dùng. Vui lòng đăng nhập lại!");
                setOpenSnackbar(true);
            }
        } else {
            setLoading(false);
    ...
    const handleRegister = async () => {
        if (!selectedActivity || !user || !user.nguoi_dung_id) {
            setSnackbarMessage("Thông tin không hợp lệ! Vui lòng đăng nhập lại.");
            setOpenSnackbar(true);
            return;
        }

        if (getActivityStatus(selectedActivity.ngay_ket_thuc) === "Hết hạn") {
            setSnackbarMessage("Hoạt động này đã hết hạn, không thể đăng ký!");
            setOpenSnackbar(true);
            return;
        }
        ...
        try {
            const payload = {
                giao_vien_id: user.giao_vien_id,
                hoat_dong_id: selectedActivity.hoat_dong_id,
                trang_thai: "Chua hoàn thành",
                minh_chung: "",
            };

            const response = await axios.post("http://localhost:5093/api/DangKyGiaoVien/dangkyhoatdonggiaovien", payload);
            ...
            if (error.response) {
                ...
                setSnackbarMessage(errorMsg); // Sẽ hiển thị "Thông báo" nếu backend đã sửa
                setOpenSnackbar(true);
            } else {
                setSnackbarMessage("Thông báo"); // Thay "Không thể kết nối đến server..." thành "Thông báo"
                setOpenSnackbar(true);
            }
        }
    };
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
                {loading ? (
                    ...
                    <Typography variant="h6">
                        Xin chào, {user.ho_ten_giao_vien || "Người dùng"}! Tổng điểm: {user.diem || 0}
                    </Typography>
                ) : (
                    ...
                    {selectedActivity && (
                        <>
                            ...
                            <Typography variant="body2">
                                <strong>Thời gian:</strong> {selectedActivity.ngay_bat_dau} -{" "}
                                {selectedActivity.ngay_ket_thuc}
                            </Typography>
                            <Typography variant="body2">
