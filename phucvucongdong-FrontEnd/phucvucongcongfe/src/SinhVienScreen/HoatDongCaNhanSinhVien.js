...
const HoatDongCaNhanSinhVien = () => {
    ...
    const validateForm = () => {
        const newErrors = {};
        if (!form.ten_hoat_dong) newErrors.ten_hoat_dong = 'Tên hoạt động là bắt buộc';
        else if (form.ten_hoat_dong.length < 3) newErrors.ten_hoat_dong = 'Tên hoạt động phải có ít nhất 3 ký tự';
        if (!form.mo_ta) newErrors.mo_ta = 'Mô tả là bắt buộc';
        else if (form.mo_ta.length < 10) newErrors.mo_ta = 'Mô tả phải có ít nhất 10 ký tự';
        if (!form.ngay_bat_dau) newErrors.ngay_bat_dau = 'Ngày bắt đầu là bắt buộc';
        if (!form.ngay_ket_thuc) newErrors.ngay_ket_thuc = 'Ngày kết thúc là bắt buộc';
        else if (form.ngay_ket_thuc && form.ngay_bat_dau && new Date(form.ngay_ket_thuc) <= new Date(form.ngay_bat_dau))
            newErrors.ngay_ket_thuc = 'Ngày kết thúc phải sau ngày bắt đầu';
        if (!form.tieu_chi_id) newErrors.tieu_chi_id = 'Vui lòng chọn tiêu chí';
        return newErrors;
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const dto = { ...form, nguoi_dung_id: parseInt(nguoi_dung_id) };
            const response = await axios.post('http://localhost:5093/api/SinhVien/sinhvientaohoatdongcanhan', dto);
            setMessage('✅ Tạo hoạt động thành công!');
            setKetQua(response.data.data);
            setForm({ ten_hoat_dong: '', mo_ta: '', ngay_bat_dau: '', ngay_ket_thuc: '', tieu_chi_id: '' });
            setErrors({});
        } catch (error) {
        ...
        <>
            ...
            <div className="page-container">
                <div className="form-container">
                    <h2 className="form-title">Tạo Hoạt Động Cá Nhân</h2>
                    ...
                    <form onSubmit={handleSubmit} className="form-group">
                        <div className="form-group">
                            ...
                            <input
                                ...
                                className={`form-input ${errors.ten_hoat_dong ? 'input-error' : ''}`}
                            />
                            {errors.ten_hoat_dong && <p className="error-text">{errors.ten_hoat_dong}</p>}
                        </div>
                        ...
                        <div className="form-group">
                            ...
                            <textarea
                                ...
                                rows="4"
                                className={`form-input ${errors.mo_ta ? 'input-error' : ''}`}
                            />
                            {errors.mo_ta && <p className="error-text">{errors.mo_ta}</p>}
                        </div>
                        ...
                        <div className="form-group grid">
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={handleChange}
                                    className={`form-input ${errors.ngay_bat_dau ? 'input-error' : ''}`}
                                />
                                {errors.ngay_bat_dau && <p className="error-text">{errors.ngay_bat_dau}</p>}
                            </div>
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={handleChange}
                                    className={`form-input ${errors.ngay_ket_thuc ? 'input-error' : ''}`}
                                />
                                {errors.ngay_ket_thuc && <p className="error-text">{errors.ngay_ket_thuc}</p>}
                            </div>
                        </div>
                        ...
                        <div className="form-group">
                            ...
                            <select
                                ...
                                onChange={handleChange}
                                className={`form-input ${errors.tieu_chi_id ? 'input-error' : ''}`}
                            >
                            ...
                            {errors.tieu_chi_id && <p className="error-text">{errors.tieu_chi_id}</p>}
                        </div>
...
export default HoatDongCaNhanSinhVien;